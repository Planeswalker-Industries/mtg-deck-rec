-- Inside the function, the eligible-cards CTE is referenced three times, so Postgres materialized it: an unindexed
-- temporary result that every card tagging was joined against (3–4.5s per call, over the 3s anonymous API timeout).
-- NOT MATERIALIZED inlines it at each use, so those joins become primary-key lookups on cards again.
