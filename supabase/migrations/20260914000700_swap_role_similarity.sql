-- Exact-tag similarity alone rewards the same mechanism over the same job: Lightning Bolt ("burn any", "spot removal")
-- matched Blazing Torch at 1.0 but Swords to Plowshares at 0.5, although both are removal. Similarity is now half
-- exact tag match and half role overlap, where roles are the allowlisted functional roots (removal, ramp, ...).
-- Pool ordering mirrors the app blend without play-rate data: tag 0.4, staple 0.2, mana value 0.1 (normalized).
...
as $$
  ...
  target_roles as (
    select distinct tc.ancestor_id as role_id
    from target_tags tt
    join public.tag_closure tc on tc.descendant_id = tt.tag_id
    join allowed_roots r on r.id = tc.ancestor_id
  ),
  role_count as (
    select greatest(count(*), 1)::real as n from target_roles
  ),
  candidate_roles as (
    select ct.card_id, count(distinct tc.ancestor_id)::real as shared_roles
    from tag_scored s
    join public.card_tags ct on ct.card_id = s.card_id
    join public.functional_tags f on f.tag_id = ct.tag_id
    join public.tag_closure tc on tc.descendant_id = ct.tag_id
    where tc.ancestor_id in (select role_id from target_roles)
    group by ct.card_id
  ),
  twins as (
