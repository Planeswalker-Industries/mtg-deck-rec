export function SiteFooter() {
  return (
    <footer className="border-t text-xs text-muted-foreground">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-4 py-6">
        <p>
          Card data and images courtesy of{" "}
          <a href="https://scryfall.com" className="underline hover:text-foreground">
            Scryfall
          </a>
          ; functional tags from the Scryfall Tagger project. Prices are estimates and may be out of date.
        </p>
        <p>
          MTG Deck Rec is unofficial Fan Content permitted under the Fan Content Policy. Not approved/endorsed by
          Wizards. Portions of the materials used are property of Wizards of the Coast. ©Wizards of the Coast LLC.
        </p>
      </div>
    </footer>
  );
}
