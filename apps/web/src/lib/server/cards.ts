 ...
 * in Postgres every one of them is a heap visit in a 92 MB table. The search API holds the same columns as whole
 * documents and does its own chunking, so it answers in one round trip without touching the database at all. A
 * missing, slow or broken index falls through to the query this always ran.
 *
...
export async function fetchCardsById(db: PublicClient, ids: readonly number[]): Promise<Map<number, CardRow>> {
  ...
  const indexed = await fromIndex("Cards by id", (index) => index.cardsByID(unique));
  if (indexed) {
