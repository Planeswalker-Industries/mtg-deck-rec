/**
 * Checks the Moxfield cron route without a real deployment: the CRON_SECRET gate, the unconfigured state, and the
 * forwarded POST to the search API. Both the search API and the scheduler are faked, so this runs in CI where
 * neither exists. The Archidekt route shares the same helper; this pins the Moxfield half. Moxfield is a blocked source, so its crawl no-ops at the database - the trigger path is still worth pinning.
 *
...
const ROUTE = "https://mtg-app-psi.vercel.app/api/cron/moxfield-scrape";
