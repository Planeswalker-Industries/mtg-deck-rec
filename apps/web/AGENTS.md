...
- **Pick the image `variant` from the size it renders at.** `unoptimized` means no srcset, so `sizes` changes nothing and the browser fetches whatever `variant` names, whole: `small` 146×204 (~13 KB), `normal` 488×680 (~93 KB), `large` 672×936. A thumbnail left on the default costs seven times its weight — a page of twenty deckbuilder results was 1.83 MB of `normal` files in 112 px slots, and 1.8 s on Fast 4G between the JSON arriving and the last card appearing. **Every grid of cards is `small`**, up to `PocketGrid`'s 160 px pockets — the deck page, the deckbuilder, the collection, the journey phases, the card and commander pages. A grid is where a card is recognised by art and name, not where rules text is read, and a hundred-card deck page on `normal` is nine megabytes. `normal` is for a card shown alone at a few hundred pixels; `large` for one deliberately enlarged
- **A search-as-you-type field keeps its results on screen while the next search runs** (dimmed, `aria-busy`), and aborts the call it superseded (`CallOptions.signal`, contract v16). Replacing results with a loading state unmounts every image, including those the two searches share; leaving stale requests live starves the one that matters — measured, seventeen keystrokes pushed the last request from 94 ms to over 800 ms
- **`/deck` on a phone is budgeted for the swipe view.** `DeckBar` is two lines (commander, colours, card count and an Edit decklist pencil; bracket and collection selects on the right). Game Changers follow the bracket (`defaultIncludeGameChangers`) with no control of their own. In Swipe view each phase's heading is screen-reader only, with a one-line hint (`PhaseIntro` `brief`). The swipe cards size themselves from `100dvh` minus what surrounds them (`--swipe-chrome` for `SwipeRater`, set by the Replace phase), so re-measure at 390×844 after adding anything above them: ✓ and ✕ must stay on screen without scrolling
...
- `DeckBar` publishes its own height to the CSS custom property named by `DECK_BAR_HEIGHT_VAR` (`--deck-bar-height`, `lib/constants.ts`); other sticky surfaces below it (the deckbuilder's `CardSearchPanel` filters) offset by `var(--deck-bar-height,0px)` so they stop below the bar rather than under it
