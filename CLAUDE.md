...
- **Swipe view** (swipe rater slice 2; `components/deck/swipe-rater.tsx`, `use-swipe-rater.ts`):
  ...
  - Swipe is the default view on a first visit; the Swipe/List choice is remembered in localStorage (`lib/review-view.ts`). e2e tests that need the list view click List first.
...
- **The deck workspace** (`components/deck/deck-tool.tsx`): the deck in the middle, where you are on the left, the three jobs on the right.
  - **Cut, Add and Replace are a drill-down, not tabs.** `JobSelector` holds them; `job === null` is the deck itself, which is where the workspace opens. Tabs made all three compete with the deck for one 390px row and left the deck as a third choice.
  - **Replace has no list of its own.** A replacement is always *for a particular card*, so it shows the deck with a prompt to tap one. Don't add a standalone Replace list; the swap sheet is the answer.
  - One CSS grid, not three nested columns, so the same markup is a single stack on a phone in the mock's order — jobs, then cards — and `lg:grid-cols-[11rem_minmax(0,1fr)_17rem]` above that. The rail is column 3 but comes first in the DOM, which is what puts it above the deck on a phone.
  - **A phone collapses the selector to a Back control; a wide screen keeps all three buttons in the rail** and marks which one the middle column shows, because there is room for the rail and the job side by side. Picking the job already showing goes back to the deck — in the rail that is the only way back.
    - The two states are **CSS, not a media query**, so the server and the browser render the same markup. They never show at once, and the Back control exists only in the collapsed one, so no accessible name is ever duplicated — which is what keeps Playwright's strict mode usable.
  - `WorkspaceRail` gives each job its **top suggestion**: the lists are already ranked, so the first one is the preview, with "View all (n)" into the full list. The card itself is not a control — "View all" is the only way in. A job still loading says so rather than leaving a gap, because "still working" and "nothing to do" are different answers. Previews are wide-screen only; a phone's height goes to the cards.
  - `WorkspaceNav` is the left column: a section nav over the deck's own groups, plus the way out to decks, collection and account. Its links jump to group headings, so it only shows while the middle column is the deck. Ids come from `deck-group-id.ts`, because group keys are card types and Tagger labels and carry spaces and punctuation.
  - **Grouping lives in `useDeckGroups`, above the panel that draws it**: the section nav and the panel both need the grouping choice and its result, and neither owns the other.
...
- **`components/cards/art-backdrop.tsx`** puts a card's art crop behind a page header. An art crop carries no artist or copyright line of its own (unlike a full card image, where the line is part of the picture), so the credit is rendered beneath it and links to the card page. **With no `artist` the art isn't shown at all** — uncredited art is worse than none.
  - Its two dimming layers multiply: keep the combined alpha near 0.7 behind text. Two heavy layers reach ~0.93 and the art stops reading.
  - Used on the commander page header and the card page header. The landing hero has its own copy of the treatment, because it needs a horizontal wash (the headline sits left, the cards right) and its credit goes below the fold of the section.
  - The landing hero is **full-bleed** (`mx-[calc(50%-50vw)] w-[100vw]`), which needs `overflow-x: clip` on **both** `html` and `body` in globals.css: 100vw counts the scrollbar, so clipping only body still leaves the root overflowing. It must be `clip`, not `hidden` — `hidden` makes a scroll container and breaks the deck tool's sticky bar. Checked: page scrolled 296px with the bar still pinned at 0.
  - The hero's art layer is `absolute inset-0` against the full-bleed `<section>`, so the inner `max-w-6xl` container must **not** be `relative` or the backdrop stops at the content column.
  - The hero picks its art from `HERO_SLUGS`, an ordered list of **lands**: land art is painted as scenery, which is what a wide banner needs, where a creature portrait crops to a face. `getHeroArt` takes the list and returns the first that resolves.
  - The banner's layers are explicit: art `-z-10`, content at auto, the shadow gradient `z-10`, the jobs `z-20`. The section is `isolate` so the art stays inside it, and `overflow-hidden` so the cards are cut by the banner's foot.
  - The jobs list uses `md:ml-auto` with a `md:max-w-` cap, not `justify-between`, so it is clamped to the hand's width and starts where the cards start. It is pinned by margin rather than a flex sibling because the credit is a Suspense child that renders null while it loads and whenever the art is missing.
  - The card fan is scaled from its top edge (`md:origin-top md:scale-*`) rather than given a bigger box: the cards reach down behind the jobs without the wrapper growing, so the banner keeps its height. They are decorative, so scaling costs nothing.
  - The art credit sits **below** the banner, not inside it: pinned bottom-left it collided with "No account needed." once the jobs row rose into the cards.
  - Art crops are only 626px wide, so at 1920 the banner upscales about 3x. The dimming hides it, but don't treat the backdrop as detail — it is atmosphere.
  - **The landing page's art is the one database read on that page**, so it is wrapped in `<Suspense fallback={null}>` and `getHeroArt` returns null on any error. The page renders without a backdrop rather than failing; checked by pointing `HERO_SLUG` at a missing card (200, headline intact, no credit, no server error). Keep it that way — the landing page should not be able to break when the database is down.
- **`PocketGrid`'s columns answer to its container, not the viewport** (`@container` plus `@md:`/`@xl:`/`@3xl:`). The workspace's middle column is about half the page, where viewport breakpoints asked for six cards in the room for four. Any new place that narrows the grid gets the right count for free; a viewport breakpoint here would not.
