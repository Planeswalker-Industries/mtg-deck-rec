...
docker compose -f docker-compose.typesense.yml up -d     # local search index on http://localhost:56325 (its own port, beside Supabase's 56321-56324); the VPS runs deploy/typesense/
...
- **Search index (Typesense, self-hosted on the owner's VPS):** the highest-volume reads are document lookups, and Postgres charges a heap visit for each. `docs/roadmap/typesense-plan.md` says what moves and why; `docs/roadmap/typesense-ops.md` is the runbook.
  ...
  - **Two keys.** The worker and the sync workflow hold `TYPESENSE_ADMIN_KEY`; the app only ever gets `TYPESENSE_SEARCH_KEY`, scoped to `documents:search` and `documents:get`. Search stays server-side behind `/api/cards/search` so it keeps the `search` rate-limit bucket.
